...
def load_and_process_data(file_path):
    ...
    if len(df) == 0:
        ...
        # ====================================================
        # 新增步骤: 零损耗数据平滑 (Zero-Loss Smoothing)
        # ====================================================
    ...
    cols_to_smooth = [
        ...
        # 2. 设定平滑参数
        # Window=3: 既能滤除尖峰噪声，又不会造成太大的信号滞后
    SMOOTH_WINDOW = 3
    ...
    #
    #     # 4. 在每个片段内部(groupby)进行变换(transform)
    #     # transform 保证输出的形状和输入完全一致，直接赋值回原列
    # df[cols_to_smooth] = df.groupby('segment_id')[cols_to_smooth].transform(smooth_func)
    #
    # print(f"平滑处理完成。数据行数保持不变: {len(df)}")
    def smooth_func(x):
...
if __name__ == "__main__":
    # 确保文件名和路径正确
    try:
        X_data, Y_data,Phys_data, scaler_x, scaler_y = load_and_process_data('data/train_dataset.csv')
