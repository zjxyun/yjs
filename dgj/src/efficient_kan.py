...
class KANLinear(nn.Module):
    ...
    def regularization_loss(self, regularize_activation=1.0, regularize_entropy=1.0):
        ...
        """
        l1_fake = self.spline_weight.abs().mean(dim=-1)
    ...
    def forward(self, x: torch.Tensor, update_grid=False):
        for index, layer in enumerate(self.layers):
            if update_grid:
                layer.update_grid(x)
            x = layer(x)
            # KAN 中间层需要把输出压缩到 [-1, 1] 才能让下一层的 B-spline 工作良好
            # 最后一层不需要 tanh (回归问题)
            if index < len(self.layers) - 1:
                # x = torch.tanh(x)
                # 提示：原版KAN论文建议使用Tanh或者不做限制由update_grid自动适应
                # 但Efficient-KAN通常不需要显式的Tanh，如果使用update_grid功能
                # 简单起见，这里加上一个 tanh 防止数值发散是安全的
                x = torch.tanh(x)
        return x
