...
from src.module_tjepa import TJEPA_MLP_PIDL
...
def train_pidl():
    ...
    model = TJEPA_MLP_PIDL().to(device)
    # 优化器配置：可以同时微调 T-JEPA 和训练 DNN
    optimizer = torch.optim.Adam([
        {'params': model.tjepa.parameters(), 'lr': 1e-5},  # T-JEPA 微调学习率 (极小)
        {'params': model.get_net_parameters(), 'lr': 0.001},  # DNN 网络学习率
        {'params': model.get_phy_parameters(), 'lr': 0.01, 'weight_decay': 1e-4}  # 物理参数
    ])
